...
def export_rdf_turtle(corpus_df, metadata_bundle, ontology_bundle):
    g = Graph()
    g.bind("base", BASE)
    ...
    if isinstance(ontology_bundle, dict):
        classes = ontology_bundle.get("classes", [])
...
def export_jsonld(corpus_df, metadata_bundle):
    graph_items = []

    graph_items.append({
        "@id": "http://example.org/kg/corpus/main",
        "@type": "Dataset",
        "name": metadata_bundle.get("title", "Corpus") if isinstance(metadata_bundle, dict) else "Corpus",
        "description": metadata_bundle.get("summary", "") if isinstance(metadata_bundle, dict) else "",
        "keywords": metadata_bundle.get("keywords", []) if isinstance(metadata_bundle, dict) else [],
    })

    for _, row in corpus_df.iterrows():
        chunk_id = f"http://example.org/kg/chunk/{_safe_uri(row['chunk_id'])}"
        mentions = [f"http://example.org/kg/entity/{_safe_uri(e)}" for e in row.get("entities", [])]
